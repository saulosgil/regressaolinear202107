...
# EQM = mean((y - yh)^2).
y <- cars_com_predicoes$dist # SG-coluna com valor de y
yh <- cars_com_predicoes$.fitted # SG-coluna com valor previstos (B0+B1*x)
...
# SG-O resultado do EQM do summary apresenta os valores mínimo, máximo e quartis sem elevar oo EQ de cada valor ao quadrado. Quando eu rodo um quantile() do elemento EQM, os valores são equivalentes aos observados no summary.
```
...
xbarra <- mean(cars$speed)  #média de X
ybarra <- mean(cars$dist) #média de y
...
```{r}
summary(melhor_reta) # SG- Conforme observado a speed se mostrou significativamente associada com a variavél dist. 
```
